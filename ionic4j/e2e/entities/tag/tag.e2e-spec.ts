...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    await element.all(by.css('ion-item'))
      .filter(async el => (await el.element(by.css('h2')).getText()) === 'Tag').first().click();

    tagComponentsPage = new TagComponentsPage();
    await browser.wait(ec.visibilityOf(tagComponentsPage.title), 5000);
    expect(await tagComponentsPage.getTitle())
  ...
  it('should create Tag', async () => {
    ...
    await browser.wait(ec.visibilityOf(tagUpdatePage.pageTitle), 1000);
    expect(await tagUpdatePage.getPageTitle())
      .toEqual(SUBCOMPONENT_TITLE);

    await tagUpdatePage.setNameInput(name);

    await tagUpdatePage.save();
    await browser.wait(ec.visibilityOf(tagComponentsPage.title), 1000);
    expect(await tagComponentsPage.getTitle())
      .toEqual(COMPONENT_TITLE);
    expect(await tagComponentsPage.getEntitiesNumber())
  ...
  it('should view the last Tag', async () => {
    ...
    expect(await tagDetailPage.getPageTitle())
      .toEqual(SUBCOMPONENT_TITLE);
